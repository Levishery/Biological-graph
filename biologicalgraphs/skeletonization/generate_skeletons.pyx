...
def TopologicalThinning(prefix, input_segmentation, skeleton_resolution=(80, 80, 80)):
    ...
    prefix_b = bytes(prefix, 'utf-8')
    lut_directory = bytes(lut_directory, 'utf-8')
    CppTopologicalThinning(prefix_b, &(cpp_skeleton_resolution[0]), lut_directory)
    ...
    CppApplyUpsampleOperation(prefix_b, &(cpp_input_segmentation[0,0,0]), &(cpp_skeleton_resolution[0]), &(cpp_output_resolution[0]))
